var expectedAssertCount = 0;
var assertCount = 0;

var isArray_ =
  Array.isArray ||
  function(obj) {
    return obj == null ? false : toString.call(obj) === "[object Array]";
  };

function expectAsserts(count) {
  parent.expectedAssertCount = count;
}
...
function assertNoException(msg, callback) {
  ...
  try {
    ...
    fail(
      args[0] +
        "expected not to throw exception, but threw " +
        e.name +
        " (" +
        e.message +
        ")"
...
function assertArray(msg, actual) {
  ...
  if (!isArray_(args[1])) {
    fail(
      args[0] + "expected to be array, but was " + prettyPrintEntity_(args[1])
    );
  }
}
...
function assertClassName(msg, className, element) {
  ...
  try {
    ...
    fail(
      args[0] +
        "expected class name to include " +
        prettyPrintEntity_(args[1]) +
        " but was " +
        actual
    );
